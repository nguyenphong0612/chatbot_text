...
class DatabaseService {
  ...
  validateConversationId(conversationId) {
    ...
    if (conversationId.startsWith('conv_')) {
      console.warn(`Custom conversation ID format detected: ${conversationId}`);
      // Tạm thời chấp nhận format này để tránh lỗi
      return conversationId;
    }
    
    console.warn(`Unknown conversation ID format: ${conversationId}`);
    // Tạm thời chấp nhận để tránh lỗi
    return conversationId;
  }
  ...
  async checkTableStructure() {
    try {
      ...
      // Thử lấy một record để kiểm tra cấu trúc
  ...
  async createConversation() {
    try {
      console.log('Creating conversation with content: []');
      const { data, error } = await supabase
        .from('conversations')
        .insert([
          {
  ...
  async addMessage(conversationId, message) {
    try {
      ...
      // Validate conversation ID
      const validId = this.validateConversationId(conversationId);
      console.log('Validated conversation ID:', validId);
      ...
      console.log('Current conversation content:', currentConversation.content);
  ...
  async updateConversationStatus(conversationId, status) {
    try {
      ...
      // Lưu status vào content JSONB thay vì cột riêng
      const { data: currentConversation, error: getError } = await supabase
        .from('conversations')
        .select('content')
        .eq('conversation_id', validId)
      ...
      // Thêm metadata vào content
      updatedContent.push({
        role: 'system',
        content: `Conversation status: ${status}`,
